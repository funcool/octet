...
And the library works with the following platforms: *jdk7*, *jdk8*, *node* (0.12.7, 4.2.0,
5.0.0).
...
It there is an example allocating a 24 bytes size byte buffer with *default*
implementation:
...
It there is an other example allocating a 24 bytes size byte buffer using *es6
...
At this moment, _octet_ does not incluedes facilities for create repeated
compositions, but it can be solved using normal clojure primitives for it.
